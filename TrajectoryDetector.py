...
class TrajectoryDetector:
    def __init__(self, image):
        ...
        self.__trajectory = []
        self.__sorted_trajectory = [self.__start_point.getPoint3DArray()]
    ...
    def getTrajectory(self):
        return self.__sorted_trajectory

    def sortPoints(self) -> None:
        while self.__trajectory:
            self.__trajectory.sort(key=lambda point3d: np.sqrt(
                (point3d[0] - self.__sorted_trajectory[-1][0]) ** 2 + (
                        point3d[1] - self.__sorted_trajectory[-1][1]) ** 2 + (
                        point3d[2] - self.__sorted_trajectory[-1][2]) ** 2))
            self.__sorted_trajectory.append(self.__trajectory[0])
            self.__trajectory.pop(0)

    def calculateYaw(self) -> None:
        for i in range(1, len(self.__sorted_trajectory)):
            diff_x = self.__sorted_trajectory[i][0] - self.__sorted_trajectory[i - 1][0]
            diff_y = self.__sorted_trajectory[i][1] - self.__sorted_trajectory[i - 1][1]
            # print("diff_x: ", diff_x, " diff_y: ", diff_y)
            self.__sorted_trajectory[i][5] = np.round(np.arctan2(diff_y, diff_x) * 180 / np.pi, 2)
        ...
        for h in range(int(cube_cy), int(height), 2 * int(cube_cy)):
            for w in range(int(cube_cx), int(width), 2 * int(cube_cx)):
                ...
                point3d = Point3D(self.scaleValueIntoWorldPoint(w, "x"),
                                  ...
                                  color)
                self.__trajectory.append(point3d.getPoint3DArray())
        print("trajectory len: ", len(self.__trajectory))
